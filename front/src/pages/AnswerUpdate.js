    ...
    useEffect(() => {
        const fetchView = async () => {
            try {
                ...
                setContent(res.data.answerVO.content);
            } catch (err) {
                setQuestionVO(null);
            } finally {
                setLoading(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post(
                `http://${API_IP}/admin/qna/update.do`,
                { question_no, content },
                {
            ...
            if (err.response) {
                if (err.response.status === 401) {
                    ...
                    const serverMessage = typeof err.response.data === 'string' ? err.response.data : '답변 수정에 실패했습니다.';
                    alert(serverMessage);
                }
            } else if (err.request) {
